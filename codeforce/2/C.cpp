...
class Geometry
{
    public:
        ...
        {
            long double a = ((x2 - x1) * (x3 - x1) + (y2 - y1) * (y3 - y1))
...
{
    public:
        int type;    // 1-Line, 2-Circle
        LD a, b, c;  // Line ax+by+c;
        LD g, h, r;
};
...
int main()
{
    ...
    for (int i = 0; i < pts.size(); i++)
    {
        ...
        {
            LD t = (r[j] * r[j])
                   / ((x[j] - pts[i].first) * (x[j] - pts[i].first) + (y[j] - pts[i].second) * (y[j] - pts[i].second));
    ...
    {
        cout.precision(14);
        cout << fixed << ret.first << " " << ret.second << endl;
